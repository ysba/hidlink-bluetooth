...
static void serial_task() {
    ...
    while(1) {
        if(xQueueReceive(serial.tx_queue, &frame, pdMS_TO_TICKS(1000)) == pdTRUE) {
            ESP_LOGI(TAG, "ok");
...
void serial_start() {
    ...
    if((err = uart_driver_install(HIDLINK_UART_PORT_NUM, HIDLINK_UART_BUF_SIZE, 0, 0, NULL, intr_alloc_flags)) != ESP_OK) {
